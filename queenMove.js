...
var moves_positive_z1: RaycastHit[] = [];
var moves_negative_z1: RaycastHit[] = [];
var moves_positive_x1: RaycastHit[] = [];
var moves_negative_x1: RaycastHit[] = [];
var self: RaycastHit[] = [];
...
public function select(){
	...
	for(m = 5; m < 40; m = m+5){
	moves_positive_z1 += Physics.RaycastAll(transform.position + new Vector3(m,10,m),new Vector3(0,-1,0), 50F);
	moves_negative_z1 += Physics.RaycastAll(transform.position + new Vector3(-m,10,m),new Vector3(0,-1,0), 50F);
	moves_positive_x1 += Physics.RaycastAll(transform.position + new Vector3(m,10,-m),new Vector3(0,-1,0), 50F);
	moves_negative_x1 += Physics.RaycastAll(transform.position + new Vector3(-m,10,-m),new Vector3(0,-1,0), 50F);
	}

		if((!defendingKingRockZ) && (!defendingKingRockX)){
		//move rocks on positive z
		if(!defendingKingBishopX){
		directionsSelect(moves_positive_z1);		
		//move rocks on negative z
		directionsSelect(moves_negative_x1);
		}
		//move rocks on positive x
		if(!defendingKingBishopZ){
		directionsSelect(moves_positive_x1);		
		//move rocks on negative x	
		directionsSelect(moves_negative_z1);
		}
		}
	
}

function directionsSelect(moves : RaycastHit[]){
...
function directionsDeselect(moves : RaycastHit[]){
			for (var i: int = 0; i < moves.Length; i++) {
				if(moves[i].transform.GetComponent.<Move>()){
					if ((game.GetComponent(gameManager).blackCheck) || (game.GetComponent(gameManager).whiteCheck)){ //se esta em check
...
moves_negative_x = [];
 ...
 // -------------------------------------------------bishop Moves ---------------------------------------------------------------
	...
	if((!defendingKingRockZ) && (!defendingKingRockX)){
		if(!defendingKingBishopX){
	//move rocks on positive z
	directionsDeselect(moves_positive_z1);
	//move rocks on negative z
	directionsDeselect(moves_negative_x1);
	}
	//move rocks on positive x
	if(!defendingKingBishopZ){
	directionsDeselect(moves_positive_x1);		
	//move rocks on negative x
	directionsDeselect(moves_negative_z1);	
	}
	}
